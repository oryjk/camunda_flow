...
class ListenerTest extends Specification {
  ...
  private val source =
    """<?xml version="1.0" encoding="UTF-8"?>
      ...
      |      <bpmn:outgoing>SequenceFlow_0jfsbi6</bpmn:outgoing>
      |    </bpmn:userTask>
      |    <bpmn:sequenceFlow id="SequenceFlow_1sip09p" sourceRef="StartEvent_1" targetRef="Task_1xem4je" />
      |    <bpmn:sequenceFlow id="SequenceFlow_0jfsbi6" sourceRef="Task_1xem4je" targetRef="Task_03gf2j6" />
      ...
      |        </camunda:taskListener>
      |      </bpmn:extensionElements>
      |      <bpmn:incoming>SequenceFlow_0jfsbi6</bpmn:incoming>
      ...
      |        </bpmndi:BPMNLabel>
      |      </bpmndi:BPMNEdge>
      |      <bpmndi:BPMNEdge id="SequenceFlow_0jfsbi6_di" bpmnElement="SequenceFlow_0jfsbi6">
      |        <di:waypoint x="387" y="185" />
      |        <di:waypoint x="463" y="185" />
      |        <bpmndi:BPMNLabel>
      |          <dc:Bounds x="425" y="163.5" width="0" height="13" />
      |        </bpmndi:BPMNLabel>
      |      </bpmndi:BPMNEdge>
  ...
  "ListenerTest" should {
    ...
    "complete task" in {
      ...
      tasks.size() should beEqualTo(1)
      tasks.asScala
        .foreach(task => {
          task.setAssignee("carlwang")
          val userTask = taskService.createTaskQuery().taskCandidateUser("department").singleResult()
          logger.info("task id {},task {}, userTask {}", task.getId, task, userTask)
          taskService.complete(task.getId)
        })
      tasks = taskService.createTaskQuery().list()
      val userTask = taskService.createTaskQuery().taskCandidateUser("department").singleResult()
      userTask must beNull
      tasks.asScala.foreach(task => {
        logger.info("task {}", task)
      })
      taskService.createTaskQuery().list().size() shouldEqual 1
    }
