...
import org.camunda.bpm.engine.{IdentityService, _}
...
class UserProcessInstanceServiceImplTest extends Specification {
  ...
  private val userProcessInstanceService: UserProcessInstanceServiceImpl = new UserProcessInstanceServiceImpl(runtimeService,
    identityService,
    taskService,
    repositoryService)

  "userProcessInstanceService" should {
    val deploy = repositoryService.createDeployment.addString("user.bpmn20.xml",
      """<?xml version="1.0" encoding="UTF-8"?>
        ...
        |  </bpmndi:BPMNDiagram>
        |</bpmn:definitions>
        |""".stripMargin).name("复星决策流程").deploy


    runtimeService.startProcessInstanceByKey("Process_1")
    "query_process_instance_by_invalid_user_id" in {
      val instanceApis = userProcessInstanceService.queryProcessInstance("xxxx")
      CollectionUtils.isEmpty(instanceApis.asJava) must be equalTo true
    }

    "query_process_instance_by_valid_user_id" in {
      val instanceApis1 = userProcessInstanceService.queryProcessInstance("user1")
      instanceApis1.size must be equalTo 1
      val instanceApis2 = userProcessInstanceService.queryProcessInstance("user2")
      instanceApis2.size must be equalTo 1
    }

    "query_task_by_user_id" in {
      val tasks = taskService.createTaskQuery().taskCandidateUser("user1").list()
      tasks.size must be equalTo 1
      taskService.setVariable(tasks.get(0).getId, "agree", taskService.getVariable(tasks.get(0).getId, "agree").asInstanceOf[Int] + 1)
      var count = taskService.getVariable(tasks.get(0).getId, "agree").asInstanceOf[Int]
      count must be equalTo 1
      taskService.setVariable(tasks.get(0).getId, "agree", taskService.getVariable(tasks.get(0).getId, "agree").asInstanceOf[Int] + 1)
      count = taskService.getVariable(tasks.get(0).getId, "agree").asInstanceOf[Int]
