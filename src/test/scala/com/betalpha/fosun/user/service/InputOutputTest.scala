...
@SpringBootTest(classes = Array(classOf[BackendFosunApplication]), webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@Slf4j
class InputOutputTest extends Specification {

  @Autowired
  var userProcessInstanceService: UserProcessInstanceService = _
  ...
  @Autowired
  var barTaskService: BarTaskServiceImp = _
  ...
  def initial: Unit = {
    ...
    logger.info("barTaskService {}",barTaskService)
  }

  @Test
  def get_input_parameter_test: Unit = {

    taskService.createTaskQuery().list().asScala.map(task => {
      logger.info("task {}", task)
      task.getOwner
      val value = taskService.getVariable(task.getId, "rule")
